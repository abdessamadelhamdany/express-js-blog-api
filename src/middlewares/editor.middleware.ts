...
/**
 * Verify that the authenticated user is an administrator/editor.
 */
export default async function editor(req: Request, res: Response, next: NextFunction) {
  return await auth(req, res, async () => {
    try {
      ...
      if (req.user && req.user.role === UsersInterfaces.UserRole.EDITOR) {
        return next();
      }
